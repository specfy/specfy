...
export async function getUsage(org) {
    const res = await prisma.$queryRaw(Prisma.sql `SELECT
      ...
      LIMIT 1`);
    const counts = res[0];
    const plan = Object.values(v1).find((p) => p.id === org.currentPlanId) ||
...
export async function createFreeSubscription({ org, me, }) {
    if (!BILLING_ENABLED) {
        return;
    }
