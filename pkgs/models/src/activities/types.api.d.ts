...
export type ApiActivity = Omit<DBActivity, 'projectId' | 'targetComponentId' | 'targetDocumentId' | 'targetPolicyId' | 'targetRevisionId' | 'targetUserId' | 'userId'> & {
    ...
    targetDocument?: Pick<ApiDocument, 'id' | 'name' | 'slug' | 'type'> | undefined;
    targetRevision?: Pick<ApiRevision, 'id' | 'locked' | 'merged' | 'name' | 'status'> | undefined;
    targetPolicy?: Pick<ApiPolicy, 'id' | 'name'> | undefined;
};
export type ApiActivityGrouped = ApiActivity & {
    childrens?: ApiActivity[];
};
