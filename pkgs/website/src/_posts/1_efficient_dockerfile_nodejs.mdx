...
<Banner type="info">To build from scratch everytime you need to `docker system prune` and `docker build --pull --no-cache ./`</Banner>
...
From scratch: 54.1s
Code source modified: 17s
Cached: 1.0
Size: 613MB

We don't save on the final image size since all `devDependencies` were already ignored, but we saved a few seconds (about 5seconds or 10% decrease) on the initial install time and we save on intermediate layers size. This step is definitely far less impacting but always good to take.
...
- Removing the source code is usally not that impactful. By adding this, I saved around 2MB.
...
- [depot.dev](http://depot.dev), while being very promising is a payed/remote solution.
