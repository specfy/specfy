---
id: 1
slug: efficient-dockerfile-nodejs-in-7-steps
title: Building efficient NodeJS Docker image
socialImage: posts/1/building-efficient-dockerfile-nodejs.png
...
This is the initial step. We won't go too much into details but it's very important to ignore some folders and can drastically change the build time or size. Especially if you are building locally.
An other important aspect is to not push your [secrets in the image](https://www.bleepingcomputer.com/news/security/thousands-of-images-on-docker-hub-leak-auth-secrets-private-keys/). Even if you delete them inside, they will live in the intermediate layers.
...
After this basic step, we can start optimizing the build by using Slim or Alpine image.
...
That was an easy step and we managed to reduce our image size by 36%. Classic distribs contains a lot of unecessary tools and binaries that are removed in Slim distribs.
...
<div className="lg:flex lg:flex-row">
  <p className="w-full lg:w-[400px] flex-shrink-0 lg:mr-4">
    <a href="/posts/1/multi-stage-explanation.png" target="_blank"><img src="/posts/1/multi-stage-explanation.png" alt="An explanation of why use multi-stage build the way we do" /></a>
  </p>
  <div>
...
- **Deleting files** during the build will **not save size** unless you are using Multi-Staged build.
